...
const TERMS = [
  { word: 'HODL', meaning: 'Hold On for Dear Life — never sell, no matter what.', tweet: '"Just HODLing through this 80% dip. This is fine. 🔥"', color: '#00FF94' },
  { word: 'FOMO', meaning: 'Fear Of Missing Out — buying because everyone else is pumping.', tweet: '"Bitcoin up 40% today. My hands are shaking. This is FOMO. Help."', color: '#FFD700' },
  { word: 'FUD', meaning: 'Fear, Uncertainty, and Doubt — negative sentiment, often spread intentionally.', tweet: '"Ignore the FUD ser. They said Bitcoin was dead 474 times."', color: '#FF3366' },
  { word: 'DYOR', meaning: "Do Your Own Research — never trust blindly. Verify everything.", tweet: '"NFA but DYOR before aping in. (This is also NFA.)"', color: '#0099FF' },
  { word: 'NFA', meaning: "Not Financial Advice — the crypto community's favorite legal disclaimer.", tweet: '"This coin will 100x NFA 👀"', color: '#9945FF' },
]

const QUIZ = [
  { term: 'HODL', q: 'What does HODL mean in crypto?', options: ['Hold On for Dear Life — never sell', 'A type of crypto wallet', 'High Output Digital Ledger', 'A trading strategy for day traders'], correct: 0, fact: "Started as a drunk typo of \"hold\" in a 2013 Bitcoin forum post. Now it's a lifestyle." },
  { term: 'FOMO', q: "FOMO in crypto means you're buying because...", options: ['You did deep research', 'Fear Of Missing Out — everyone else is pumping', 'A fundamental analysis signal', 'You found a rug pull early'], correct: 1, fact: 'FOMO buyers almost always buy the top. Classic ngmi behavior.' },
  { term: 'FUD', q: 'What is FUD?', options: ['A type of blockchain consensus', 'Fear, Uncertainty, and Doubt — negative sentiment', 'Fully Unlocked Distribution', 'A DeFi protocol'], correct: 1, fact: 'FUD spreaders are either scared, short-selling, or just wrong. Sometimes all three.' },
  { term: 'DYOR', q: 'What does DYOR stand for?', options: ['Do Your Own Research', 'Deploy Your Own Router', 'Digital Yield On Returns', 'Decentralized Yield Optimization Resource'], correct: 0, fact: 'The most important phrase in crypto. If someone says "trust me ser" without DYOR — run.' },
  { term: 'NFA', q: 'When someone says NFA, what do they mean?', options: ['New Financial Asset', "Not Financial Advice — don't sue me", 'Non-Fungible Asset', 'Network Fee Adjustment'], correct: 1, fact: "Every crypto influencer's favorite legal shield. \"Buy this coin NFA\" = still not advice." },
]

function ConfettiBurst() {
  const colors = ['#00FF94', '#FFD700', '#FF3366', '#0099FF', '#9945FF', '#F7931A']
  return (
    ...
    <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.25 }}>
      ...
      <div className="flex flex-col gap-3 mb-4">
        {q.options.map((opt, i) => {
          ...
          if (picked !== null) {
            if (i === q.correct) { border = `1px solid ${ACCENT}`; bg = `${ACCENT}1A`; col = ACCENT }
