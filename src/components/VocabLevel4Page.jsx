...
const QUIZ = [
  {
    term: 'CA',
    q: 'In crypto, "CA" stands for:',
    options: ['Crypto Analyst', 'Contract Address — the on-chain ID of a token', 'Centralized Authority', 'Capital Allocation'],
    correct: 1,
    fact: 'Drop the CA in the group chat and watch everyone ape in. Always verify the CA before buying.',
  },
  {
    term: 'GAS',
    q: 'What is "gas" in blockchain terms?',
    options: ['Energy used to mine Bitcoin', 'The fee paid to process a transaction on-chain', 'A privacy layer on Ethereum', 'A type of stablecoin'],
    correct: 1,
    fact: 'Gas fees on Ethereum can cost more than your actual transaction. Peak blockchain UX.',
  },
  {
    term: 'SEED PHRASE',
    q: 'What is a seed phrase?',
    options: ['A password for a crypto exchange', '12-24 words that control access to your wallet forever', 'A smart contract initialization code', 'A DeFi yield farming term'],
    correct: 1,
    fact: 'Lose your seed phrase = lose everything. Screenshot it = get hacked. Write it on paper. Store it offline.',
  },
  {
    term: 'WALLET',
    q: 'A crypto wallet actually stores:',
    options: ['Your crypto coins', 'Private keys that prove you own crypto on the blockchain', 'A password to the exchange', 'NFT image files'],
    correct: 1,
    fact: 'Your wallet doesn\'t store crypto — it stores the keys to prove ownership on the blockchain. Not your keys, not your coins.',
  },
  {
    term: 'BRIDGE',
    q: 'What does a blockchain bridge do?',
    options: ['Connects two different blockchains to transfer assets', 'Speeds up Bitcoin transactions', 'A way to stake ETH for rewards', 'A type of hardware wallet'],
    correct: 0,
    fact: 'Bridges let you move assets between chains (ETH to Solana, etc). Also a top target for hackers.',
  },
]
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 20px 64px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: `1px solid ${ACCENT}44`, borderLeft: `4px solid ${ACCENT}`, borderRadius: 12, padding: 32, marginBottom: 32 }}>
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              <div className="font-mono text-xs mb-1" style={{ color: ACCENT }}>VOCAB · VOL. 4</div>
              <h1 className="font-syne font-black text-2xl" style={{ color: 'var(--text-primary)' }}>Technical Terms</h1>
            </div>
          </div>
          <p className="font-mono text-sm" style={{ color: 'var(--text-secondary)' }}>CA, Wallet, Gas, Seed phrase, Bridge — the mechanics every anon must understand.</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {phase === 'quiz' ? (
            ...
            <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ background: 'var(--bg-card)', border: `1px solid ${ACCENT}33`, borderRadius: 12, padding: 32 }}>
              <VocabResults score={finalScore} earnedPts={earnedPts} onRetry={handleRetry} onNext={() => navigate('/vocab/5')} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
