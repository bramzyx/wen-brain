...
const QUIZ = [
  {
    term: 'DIAMOND HANDS',
    q: 'Someone with "diamond hands" in crypto:',
    options: ['Sells at the first sign of a dip', 'Holds through all volatility, no matter what', 'Only buys stablecoins', 'Trades with leverage'],
    correct: 1,
    fact: '💎🙌 Diamond hands hold through 80% drawdowns. Legendary or delusional. Sometimes both.',
  },
  {
    term: 'PAPER HANDS',
    q: 'Having "paper hands" means you:',
    options: ['Only invest in NFTs', 'Panic sell during dips and market downturns', 'Trade using paper trading accounts', 'Hold stablecoins exclusively'],
    correct: 1,
    fact: 'Paper hands sell at the first red candle. Then watch the price moon without them. Classic.',
  },
  {
    term: 'WEN MOON',
    q: '"Wen moon" is crypto slang for:',
    options: ['A lunar calendar for trading signals', 'When will this coin\'s price go up significantly?', 'A DeFi yield optimization strategy', 'Wrapped Ethereum Network Moon token'],
    correct: 1,
    fact: '"Wen moon ser" is the eternal question of every bag holder. The answer is always "soon."',
  },
  {
    term: 'CHAD',
    q: 'In crypto culture, calling someone a "chad" means:',
    options: ['They hold Chainlink tokens', 'They made a bold, confident, often correct move', 'They are a cryptocurrency developer', 'They run a centralized exchange'],
    correct: 1,
    fact: 'Buying Bitcoin at $1 and holding to $100k? Absolute chad move. History-making stuff.',
  },
  {
    term: 'NORMIE',
    q: 'A "normie" in crypto context is:',
    options: ['A normal-sized crypto transaction', 'Someone outside crypto culture, unaware of the space', 'A type of low-cap token', 'A Ethereum validator node'],
    correct: 1,
    fact: 'You were a normie once. Now you\'re learning vocab on WenBrain. Congratulations on your evolution.',
  },
]
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 20px 64px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: `1px solid ${ACCENT}44`, borderLeft: `4px solid ${ACCENT}`, borderRadius: 12, padding: 32, marginBottom: 32 }}>
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              <div className="font-mono text-xs mb-1" style={{ color: ACCENT }}>VOCAB · VOL. 5</div>
              <h1 className="font-syne font-black text-2xl" style={{ color: 'var(--text-primary)' }}>Meme Culture</h1>
            </div>
          </div>
          <p className="font-mono text-sm" style={{ color: 'var(--text-secondary)' }}>Diamond hands, Paper hands, Wen moon, Chad, Normie — the memes that define crypto culture.</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {phase === 'quiz' ? (
            ...
            <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ background: 'var(--bg-card)', border: `1px solid ${ACCENT}33`, borderRadius: 12, padding: 32 }}>
              <VocabResults score={finalScore} earnedPts={earnedPts} onRetry={handleRetry} onNext={() => navigate('/vocab/6')} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
