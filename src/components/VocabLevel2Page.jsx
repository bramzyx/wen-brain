...
const QUIZ = [
  {
    term: 'PUMP',
    q: 'What is a "pump" in crypto?',
    options: ['A rapid price increase, often coordinated', 'A type of hardware wallet', 'A slow steady price climb over months', 'A blockchain consensus mechanism'],
    correct: 0,
    fact: 'Pumps feel great if you\'re early. FOMO buyers at the top learn the hard way.',
  },
  {
    term: 'DUMP',
    q: 'What happens during a crypto "dump"?',
    options: ['A new coin launches', 'Price crashes hard as holders sell', 'A project announces a partnership', 'Miners increase hash rate'],
    correct: 1,
    fact: 'After every pump comes a dump. Gravity is relentless. HODL or get rekt.',
  },
  {
    term: 'RUG PULL',
    q: 'What is a rug pull?',
    options: ['A bearish chart pattern', 'Devs abandon project and take all funds', 'A major exchange delisting', 'A forced liquidation'],
    correct: 1,
    fact: 'Devs launch coin, hype it, then drain the liquidity. Exit scam 101. DYOR always.',
  },
  {
    term: 'MOON',
    q: 'When someone says a coin is "going to the moon" they mean...',
    options: ['It has a space-themed whitepaper', 'They expect the price to skyrocket massively', 'It\'s being listed on Coinbase', 'It has low market cap'],
    correct: 1,
    fact: '"Wen moon ser?" The eternal cry of every degen holding a -90% bag.',
  },
  {
    term: 'REKT',
    q: 'You got "rekt" in crypto. What happened?',
    options: ['You made a huge profit', 'You suffered a massive loss', 'Your wallet was hacked', 'You found an alpha trade'],
    correct: 1,
    fact: 'From "wrecked." Losing 80% of your portfolio in a bear market = getting absolutely rekt.',
  },
]
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 20px 64px' }}>
        ...
        <div style={{ background: 'var(--bg-card)', border: `1px solid ${ACCENT}44`, borderLeft: `4px solid ${ACCENT}`, borderRadius: 12, padding: 32, marginBottom: 32 }}>
          <div className="flex items-center gap-4 mb-3">
            ...
            <div>
              <div className="font-mono text-xs mb-1" style={{ color: ACCENT }}>VOCAB · VOL. 2</div>
              <h1 className="font-syne font-black text-2xl" style={{ color: 'var(--text-primary)' }}>Trading Talk</h1>
            </div>
          </div>
          <p className="font-mono text-sm" style={{ color: 'var(--text-secondary)' }}>Pump, Dump, Rug pull, Moon, Rekt — survival vocabulary for the trenches.</p>
        </div>
        ...
        <AnimatePresence mode="wait">
          {phase === 'quiz' ? (
            ...
            <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ background: 'var(--bg-card)', border: `1px solid ${ACCENT}33`, borderRadius: 12, padding: 32 }}>
              <VocabResults score={finalScore} earnedPts={earnedPts} onRetry={handleRetry} onNext={() => navigate('/vocab/3')} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
